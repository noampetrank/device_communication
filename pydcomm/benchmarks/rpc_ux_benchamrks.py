...
from pydcomm.utils.userexpstats import api_stats


namedtuple("ApiAction", "function_name, params".split(", "))


def single_api_call_summary(api_call, params=None, ret=None):
    res = {"function_name" : api_call.function_name,
            "call_time" : api_call.end_timme - api_call.start_time,
            "manual_fixes_count" : len(api_call.manual_times),
            "manual_fixes_time" : sum([end-start for (start,end) in api_call.manual_times]), # TBD should I ignore the first one?
            "is_exception" : api_call.is_exception,
            "is_automatic" : len(api_call.manual_times) == 0,
            "is_success" : api_call.is_exception == False
            }
...
class RPCDummyAction:

    @staticmethod
    def INSTALL():
        return ApiAction(function_name="install")

    @staticmethod
    def INIT():
        return ApiAction(function_name="__init__")
    ...
    def CREATE_CONNECTION(port=5555):
        return ApiAction(function_name="create_connection", params=[port])
...
def run_scenario(actions, so_path, rpc_factory): # TBD flag for only parameters lengths?
    ...
    try:
        for api_action in actions:
            ...
            # TBD add for versions and stuff
        stats = api_stats
    except KeyboardInterrupt:
        pass
    return {"stats" : api_stats, "params" : params, "ret_vals" : ret_vals}
...
def get_small_msgs_scenario(rep_num=1):
    return get_basic_scenario(rep_num, create_connections_num=10, input_lengths=[10 for i in range(100)])

def get_long_connection_scenario(rep_num=1):
    return get_basic_scenario(rep_num, create_connections_num=1, input_lengths=[1e6 for i in range(1000)])
...
def main():
    ...
    with BetterTee("run_{}.log".format(start_time_string)):
        with indent_printing.time():
            ...
            # TBD
            # The test .so should have a dummy excecuter with port 555 and one method:
            ...
            with open("raw_data_" + start_time_string + ".pickle", "w") as f:
                pickle.dump((rpc_factory_cls.__name__, conn_type, runs), f)
                print_run_summary(runs['rpc_factory_name'], runs['stats'], runs['params'], runs['ret_val'], print_all=False)
